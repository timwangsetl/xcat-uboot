...
#define G_OPTION_MASK		0x4000	/* generic image destination*/
#define F_OPTION_MASK		0x8000	/* generic image execution address */
#define B_OPTION_MASK		0x10000	/* generic image boot device */
...
#define GEN_IMAGE_SPI_MUST_OPT	(T_OPTION_MASK|B_OPTION_MASK|D_OPTION_MASK|E_OPTION_MASK| \
				 G_OPTION_MASK|F_OPTION_MASK|R_OPTION_MASK)

#define GEN_IMAGE_NAND_MUST_OPT	(T_OPTION_MASK|B_OPTION_MASK|D_OPTION_MASK|E_OPTION_MASK|C_OPTION_MASK| \
				 P_OPTION_MASK|G_OPTION_MASK|F_OPTION_MASK|R_OPTION_MASK)

/* 32 bit checksum */
u32 checksum32(/*u32*/void* start, u32 len, u32 csum);
u8 checksum8(/*u32*/void* start, u32 len,u8 csum);
...
int main(int argc,char** argv)
{
	...
	/* Variables for Generic Boot */
	int orig_argc = argc;
	char** orig_argv = argv;

	char *gen_boot_type = "not specified"; /* dummy value */
	BOOT_DEV gen_boot_dev = UNKNOWN;
	char *fname_image=NULL;

	unsigned int gen_dest=0,gen_exec=0;
    	struct stat fs_image_stat;
	...
	while (--argc > 0 && **++argv == '-')
	{
		while (*++*argv)
		{
			...
			case 'M': /* TWSI serial init file*/
				...
				opts |= M_OPTION_MASK;
				break;
			case 'G': /* Generic Boot image execution*/
				if (--argc <= 0) {printf("2 dest Error\n");print_usage();exit(1);}
                                gen_dest = strtoul (*++argv, (char **)&ptr, 16);
				if (*ptr) {printf("3 dest Error\n");print_usage();exit(1);}
				if (opts & G_OPTION_MASK) {printf("4 dest Error\n");print_usage();exit(1);}
				opts |= G_OPTION_MASK;
				break;
			case 'F': /* Generic Boot image execution*/
				if (--argc <= 0) {printf("5 dest Error\n");print_usage();exit(1);}
                                gen_exec = strtoul (*++argv, (char **)&ptr, 16);
				if (*ptr) {printf("6 dest Error\n");print_usage();exit(1);}
				if (opts & F_OPTION_MASK) {printf("7 dest Error\n");print_usage();exit(1);}
				opts |= F_OPTION_MASK;
				break;
			case 'B': /* image type */
				if (--argc <= 0) {print_usage();exit(1);}
                                gen_boot_type = *++argv;
				if (opts & B_OPTION_MASK) {print_usage();exit(1);}
				opts |= B_OPTION_MASK;
                                break;
			}
		}
	...
	if (opts & B_OPTION_MASK) /* for generic boot */
	{
		if (strcmp(gen_boot_type, "flash") == 0)
		{
			gen_boot_dev=BOOT_SPI;
		}
		else if (strcmp(gen_boot_type, "nand") == 0)
		{
			gen_boot_dev=BOOT_NAND;
		}
		else
		{
			printf("Error: Unknown boot device specify for Generic Image\n");
			print_usage();exit(1);
		}
	}
	...
	if (opts & T_OPTION_MASK)
	{
		...
		else if (strcmp(image_type, "gen_image") == 0)
		{
			img=IMG_GEN;

			if (strcmp(gen_boot_type, "flash") == 0)
			{
				required_opts = GEN_IMAGE_SPI_MUST_OPT;
			}
			else if (strcmp(gen_boot_type, "nand") == 0)
			{
				required_opts = GEN_IMAGE_NAND_MUST_OPT;
			}
			else
			{
				printf("Error: For Generic Image most specify boot device\n");
				print_usage();exit(1);
			}
		}
	...
	case IMG_GEN:
		/* set boot device */
		hdr.blockID = gen_boot_dev;

		/* set generic image dest/exec */
		hdr.destinationAddr = gen_dest;
		hdr.executionAddr = gen_exec;
		hdr.rsvd1 = (MV_U32)orig_argv;
		hdr.rsvd2 = (MV_U16)orig_argc;
		hdr.rsvd3 = (MV_U8)override[IMG_FILE_INDX];

		if (gen_boot_dev == BOOT_NAND)
		{
			hdr.nandPageSize = (MV_U16)nandPageSize;
			hdr.nandEccMode = (MV_U8)nandEccMode;
		}
	...
	else if(img == IMG_GEN)
	{
		if (argc == 3)
		{
			...
			if ((0 == strcmp(fname_in, fname_out))||
			   (0 == strcmp(fname_in, fname_image))||
			   (0 == strcmp(fname_out, fname_image)))
			{
				fprintf(stderr,"Error: Input and output images can't be the same\n");
				exit(1);
			}
	...
	if (img == IMG_GEN)
	{
		ImageInfo_t imageInfo; /* main image info */

		imageInfo.imageDest = image_dest;
		imageInfo.imageExec = image_exec;

		close(f_out);

		if (set_generic_boot_image(buf_in, fs_stat.st_size,&hdr, &imageInfo,
					   fname_image, fname_dram, fname_out)!=0)
		{
		...
		else
		{
			...
			if ((pre_padding)&&(padding_size))
			{
				...
				if (padding)
				{
					...
					chsum32 = checksum32(/*(u32)*/ (void*)padding, padding_size, chsum32);
				}
			...
			/* Calculate checksum */
			chsum32 = checksum32(/*(u32)*/ (void*)buf_in, (u32)((u32)fs_stat.st_size - bytesToAlign), chsum32);
			if (bytesToAlign)
			{
				memcpy(&lastDword, (buf_in + (fs_stat.st_size - bytesToAlign)) , bytesToAlign);
			}
			chsum32 = checksum32(/*(u32)*/ (void*)&lastDword, 4,chsum32);
	...
	/*if (f_image != -1)
		close(f_in);
	if (buf_image)
		munmap((void*)buf_image, fs_image_stat.st_size);*/
...
/* 8 bit checksum */
u8 checksum8(void* /*u32*/ start, u32 len,u8 csum)
{
