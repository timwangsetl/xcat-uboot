...
static ulong timestamp[2];
static ulong lastdec[2];
...
static unsigned int prevTs;
static unsigned int maxClkTicks;
...
*
*******************************************************************************/
unsigned int timerTickPassedGet(void)
{
    unsigned int currTs = mvCntmrRead(UBOOT_CNTR);

    if (prevTs >= currTs)
    {
        /* normal mode (non roll) */
        /* move timestamp fordward with absoulte diff ticks */
        tickCnt += (prevTs - currTs);
    }
    else
    {
        /* we have overflow of the count down timer */
        /* nts = tickCnt + ld + (TLV - currTs)
         * tickCnt=old stamp, ld=time that passed before passing through -1
         * (TLV-currTs) amount of time after passing though -1
         * nts = new "advancing time stamp"...it could also roll and cause problems.
         */
        tickCnt += (prevTs + maxClkTicks - currTs);
    }
...
void udelay (unsigned long usec)
{
    ...
    prevTs = mvCntmrRead(UBOOT_CNTR);  /* capure current value time */

    /* Total number of ticks in this delay */
    totalTicks = usec * (mvTclkGet() / 1000000);

    /* Max clock ticks in case of roll-over */
    maxClkTicks = MV_REG_READ(CNTMR_RELOAD_REG(UBOOT_CNTR));

    /* reset time */
    tickCnt = 0;         /* start "advancing" time stamp from 0 */
...
{
	/* reset time */
	lastdec[whoAmI()] = READ_TIMER;  /* capure current decrementer value time */
	timestamp[whoAmI()] = 0;	       /* start "advancing" time stamp from 0 */
}
...
ulong get_timer_masked (void)
{
	...
	if (lastdec[whoAmI()] >= now) {		/* normal mode (non roll) */
		/* normal mode */
		timestamp[whoAmI()] += lastdec[whoAmI()] - now; /* move stamp fordward with absoulte diff ticks */
		 ...
		 * nts = new "advancing time stamp"...it could also roll and cause problems.
		 */
		timestamp[whoAmI()]+= lastdec[whoAmI()] + (TIMER_LOAD_VAL/(mvTclkGet()/1000))- now;
	}
	lastdec[whoAmI()] = now;

	return timestamp[whoAmI()];
}
