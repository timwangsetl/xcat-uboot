...
volatile uchar	PktBuf[(PKTBUFSRX+1) * PKTSIZE_ALIGN + PKTALIGN + 64];
...
int NetLoop(proto_t protocol)
{
  ...
  int i;
	...
	if (!NetTxPacket) {
		 ...
		 */
		NetTxPacket = &PktBuf[32 /* leave place for DSA */] + (PKTALIGN - 1);
  ...
  /* 
     Repeat the loop 3 times due to network state error (tftp etc) in hsu restart.
     Never hurts.
     Giora 
  */
...
#endif
    if (eth_init(bd) < 0) {
      /* printf("%s: calling to eth_init() (which is commented now).\n", __func__); */
      eth_halt();
      return(-1);
    }
  }
