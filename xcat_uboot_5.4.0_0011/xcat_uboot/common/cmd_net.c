...
extern void ethPortCountersSimple(int port);

int do_ethPortCounters(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	gbe_num = argv[1][0] - '0';
	if (gbe_num >= featuresP->numOfGbe)
	{
		printf("Wrong GbE port number (%d).\n", gbe_num);
		return -1;
	}

	ethPortCountersSimple(gbe_num);
	return 0;
}

U_BOOT_CMD(
	ethPortCounters,	2,	1,	do_ethPortCounters,
	"Print eth GbE (Giga port) counters",
	"GbE number"
);
#endif	/* CFG_CMD_PING */
