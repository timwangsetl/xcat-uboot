...
int saveenv(void)
{
	...
	printf("Erasing Nand at 0x%08X to 0x%08X ...\n",
            CFG_ENV_OFFSET, CFG_ENV_OFFSET + CFG_ENV_SIZE);
    memset(&er_opts, 0, sizeof(er_opts));
    er_opts.offset = CFG_ENV_OFFSET;
    er_opts.length = CFG_ENV_SIZE;
    er_opts.quiet  = 1;
    if (nand_erase_opts(&nand_info[0], &er_opts))
    {
        printf("%s: nand_erase_opts failed.\n", __func__);
		return 1;
    }

	printf("Writing Nand at 0x%08X to 0x%08X ...\n",
            CFG_ENV_OFFSET, CFG_ENV_OFFSET + CFG_ENV_SIZE);
	memset(&wr_opts, 0, sizeof(wr_opts));
	wr_opts.buffer	   = (u_char *)env_ptr;
	wr_opts.length	   = CFG_ENV_SIZE;
	wr_opts.offset     = CFG_ENV_OFFSET;
	/* opts.forcejffs2 = 1; */
	wr_opts.pad	       = 1;
	wr_opts.blockalign = 1;
	wr_opts.quiet      = 1;
	if (nand_write_opts(&nand_info[0], &wr_opts))
    {
        printf("%s: nand_write_opts failed.\n", __func__);
		return 1;
    }
...
void env_relocate_spec (void)
{
	...
	if (G_nand_found_flag == 1)
	{
		/* printf("%s: before nand_read.\n", __func__); */
		/* printf("%s: G_nand_found_flag = %d.\n", __func__, G_nand_found_flag); */
	ret = nand_read(&nand_info[0], CFG_ENV_OFFSET, &total, (u_char*)env_ptr);
		/* printf("%s: after nand_read.\n", __func__); */
	}
	else
	{
		ret = -1; /* failure */
		printf("%s: nand_read was not called!!.\n", __func__);
	}

  	if (ret || total != CFG_ENV_SIZE)
	{
		/* printf("%s: calling use_default.\n", __func__); */
		return use_default();
		/* printf("%s: after use_default.\n", __func__); */
	}

	/* printf("%s: 2nd 'if'.\n", __func__); */
	if (crc32(0, env_ptr->data, ENV_SIZE) != env_ptr->crc)
	{
		/* printf("%s: calling use_default (2).\n", __func__); */
		return use_default();
		/* printf("%s: after use_default   (2).\n", __func__); */
	}
#endif /* ! ENV_IS_EMBEDDED */
}
#endif /* CFG_ENV_OFFSET_REDUND */
