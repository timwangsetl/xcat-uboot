...
static unsigned int createPageTable(void)
{
	...
	char* envCacheMode = getenv("cacheMode");
    char* env;
	unsigned int cacheMode, useCacheModeFlag;
    ...
    /*
     * check if enable page chaching (in L1 DCache)
     */
    env = getenv("enaDCache");
    ...
    {
        useCacheModeFlag = cacheMode;
        printf("\nDRAM [0-256MB] are cached by DCache.\n");
    }

	/* first region 0 MB - 0x10000000(256MB) none cacheable/bufferable */
	entry = L1Entry(SECTION, 0, 0, U_BIT|useCacheModeFlag, ALL_ACCESS);
	for (i = 0; i < 256; i++) 
	{
		*p++ = (entry | (i << 20));
	}
