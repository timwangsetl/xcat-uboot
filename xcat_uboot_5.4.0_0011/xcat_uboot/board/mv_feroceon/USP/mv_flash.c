...
MV_SFLASH_INFO* getMvFlashInfo(int index);
...
#if defined (MV_SPI_SIZE_AUTO_DETECT)
unsigned int get_sector_size(void);
unsigned int get_monitor_len(void);
#endif

/* use CFG_MAX_FLASH_BANKS_DETECT if defined */
#ifdef CFG_MAX_FLASH_BANKS_DETECT
static MV_FLASH_INFO   mvFlashInfo[CFG_MAX_FLASH_BANKS_DETECT];
extern flash_info_t	flash_info[CFG_MAX_FLASH_BANKS_DETECT];
...
/*                putc ('.');
        ...
        case MV_SPI_FLASH:
            if (prot)
            {
                if (mvSFlashWpRegionSet(&sflash, MV_WP_UPR_1OF32) != MV_OK)
                    return 1;
            }
            else
            {
...
#if defined (MV_INC_BOARD_SPI_FLASH)
void sflashInfoCopy(flash_info_t *flash_info, MV_SFLASH_INFO *pFlash)
{
    MV_U32 i;
...
//        printf("[%ldkB@%x] ", current_bank_size/1024, mflash.baseAddr);
    ...
    if ((current_bank_size = sflash_init(&sflash)) != 0)
    {
        ...
        //printf("spi [%ldkB@%x] ", current_bank_size/1024, sflash.baseAddr);
    ...
    if(sflash.manufacturerId == MV_MXIC_MANF_ID){
        flash_protect (FLAG_PROTECT_SET, CFG_MONITOR_BASE,
                        CFG_MONITOR_BASE + (CFG_MONITOR_LEN -0x40000) - 1,
                    &flash_info[0]);
    }
...
#endif
	calc_flash_size = flash_size;
	/* return flash_size; */
    return 0;
}

void mv_print_appl_partitions(void)
{
#ifdef CFG_APPL_PARTITIONS_NAME
	printf("\n");
	printf("%s partitions on flash\n", CFG_APPL_PARTITIONS_NAME);
	printf("---------------------------------\n\n");
	printf("Flash size           : %dMB\n", CFG_APPL_FLASH_SIZE/(1<<20));
	printf("u-boot               : offset=0x%08x, size=0x%08x (%dK)\n",
				 CFG_APPL_PART_UBOOT_START,
				 CFG_APPL_PART_UBOOT_SIZE,
				 CFG_APPL_PART_UBOOT_SIZE/(1<<10));

	printf("kernel/vxWorks-image : offset=0x%08x, size=0x%08x (%dK)\n",
				 CFG_APPL_PART_KERNEL_START,
				 CFG_APPL_PART_KERNEL_SIZE,
				 CFG_APPL_PART_KERNEL_SIZE/(1<<10));

	printf("Linux rootfs         : offset=0x%08x, size=0x%08x (%dK)\n",
				 CFG_APPL_PART_ROOTFS_START,
				 CFG_APPL_PART_ROOTFS_SIZE,
				 CFG_APPL_PART_ROOTFS_SIZE/(1<<10));

	printf("\n");
	printf("Single Image         : offset=0x%08x, size=0x%08x (%dK)\n",
				 CFG_APPL_PART_KERNEL_START,
				 CFG_APPL_PART_KERNEL_SIZE + CFG_APPL_PART_ROOTFS_SIZE,
				 (CFG_APPL_PART_KERNEL_SIZE + CFG_APPL_PART_ROOTFS_SIZE)/(1<<10));
	printf("\n");
...
mv_flash_erase (flash_info_t *info, int s_first, int s_last)
{
	...
	switch(mvFlashTypeFromIndexGet(s))
	{
		...
		if (mvSFstatus >= MV_WP_ALL)
		{
			...
			if (mvSFstatus > MV_WP_NONE){
			//	ret = mvSFlashSectorErase(&sflash, i);
			//}else{
				//yinnanning 20180104 for mx25L12835FMI spi-flash protect lower 7 blocks
				//if (sflash.manufacturerId == 0xc2){
					if(i < (1<< (mvSFstatus-MV_WP_NONE-1))){
						ret = MV_FAIL;
						break;
					}
				//}
...
*
*******************************************************************************/
unsigned long sflash_init (MV_SFLASH_INFO *sFlashInfoP)
{
    ...
    /* First check that SPI mode is configured */
    if (mvCtrlSpiBusModeDetect() != MV_SPI_CONN_TO_EXT_FLASH)
    {
        mvOsPrintf("%s ERROR: SPI is not routed to external flash.\n", __func__);
        return 0;
    }
#endif
    ...
    if (mvSFlashInit(sFlashInfoP, &sFlashInit) != MV_OK)
    {
    ...
    return mvSFlashSizeGet(sFlashInfoP);
}
...
/* This function should be called only after sflash_init was called */
unsigned int get_sector_size(void)
{
	return sflash.sectorSize;
}

/* This function should be called only after sflash_init was called */
unsigned int get_bootdev_size()
{
	return sflash.sectorSize * sflash.sectorNumber;
}
