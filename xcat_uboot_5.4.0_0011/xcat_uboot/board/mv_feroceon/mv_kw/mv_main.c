...
void print_dev_id(void){
...
#ifdef MIXED_24F_24G_BOARD
  mvEthPhyRegRead(0x8, 0x3, &value);
  if ( (value & 0x3f0) == 0xc0) // then we have the 1111 phy on the Rev1 board
    boardRev = "(Rev 1) ";
  else
    boardRev = "(Rev 2) ";
#else
#if defined(DB_98DX_REV1) && defined(DB_98DX4122)
  boardRev = "(Rev 1) ";
#elif defined(DB_98DX_REV2) && defined(DB_98DX4122)
  boardRev = "(Rev 2) ";
#else
  boardRev = "";
#endif
  ...
  printf(" ** MARVELL BOARD: %s %sBE (%s)\n",
         boardName, boardRev, (db_98dx4122_detected)?"detected and changed from RD-98DX3121-24G" : "configured");
  printf(" ** Linux   - BE support\n");
  printf(" ** vxWorks - BE support\n");
#else
  printf(" ** MARVELL BOARD: %s %sLE (%s)\n",
         boardName, boardRev, (db_98dx4122_detected)?"detected and changed from RD-98DX3121-24G" : "configured");
  printf(" ** Linux        - LE/BE support\n");
  printf(" ** vxWorks(elf) - LE/BE support\n");
#endif
...
ethaddr=$(ethaddr) $(netbsd_netconfig)");
  ...
  /* Linux boot arguments - gc*/

  env = getenv("rootpath");
  if(!env)
#ifdef MV_CPU_LE
    setenv("rootpath","/tftpboot/rootfs_arm-gnueabi");
#else
  setenv("rootpath","/tftpboot/rootfs_arm-gnueabi-be");
#endif

  env = getenv("consoledev");
  if(!env)
  ...
  env = getenv("fix_nfsroot_to_BE");
  if(!env)
    setenv("fix_nfsroot_to_BE","no");

  /*
   * Default is 'yes', because boards by default doesn't have pp-EEPROM.
   */
  env = getenv("switchUseSwEepromEmulation");
  if (!env)
      setenv("switchUseSwEepromEmulation", "yes");

  env = getenv("linux_parts");
  if(!env)
  {
    char buf[512];
    sprintf(buf, "mtdparts=%s:0x%08x(uboot)ro,"
            "0x%08x(kernel),0x%08x(rootfs)",
#if defined(CFG_NAND_BOOT)
            "nand_mtd",
#else
            "spi_flash",
#endif
            CFG_APPL_PART_UBOOT_SIZE,
            CFG_APPL_PART_KERNEL_SIZE,
            CFG_APPL_PART_ROOTFS_SIZE);
...
#if defined(CFG_NAND_BOOT)
    sprintf(buf, "setenv bootargs root=/dev/mtdblock2 rw rootfstype=jffs2 ip=$(ipaddr):$(serverip):$(gatewayip):$(netmask):$(hostname):$(netdev):off console=$(consoledev),$(baudrate) $(othbootargs) $(linux_parts); nboot 0x2000000 0 0x%08x; bootm 0x2000000",
            CFG_APPL_PART_KERNEL_START);
#else
    sprintf(buf, "setenv bootargs root=/dev/mtdblock2 rw rootfstype=jffs2 ip=$(ipaddr):$(serverip):$(gatewayip):$(netmask):$(hostname):$(netdev):off console=$(consoledev),$(baudrate) $(othbootargs) $(linux_parts); bootm 0x%08x",
            CFG_APPL_PART_KERNEL_START);
  ...
  if(!env)
    setenv("image_name","uImage_xcat");

  /* vxWorks boot arguments */

  env = getenv("vx_burn");
  if (!env)
    setenv("vx_burn", "mtdburn $(vx_image_name)");

  env = getenv("vx_image_name");
  if (!env)
    setenv("vx_image_name", "vxWorks.st");

  env = getenv("vx");
  if (!env)
    setenv ("vx", "setenv bootargs mgi(0,0) host:VxWorks h=$(serverip) e=$(ipaddr):FFFF0000 u=anonymous pw=target; setenv bootaddr 0x700; setenv loadaddr 0x3000000; tftpboot $(loadaddr) $(vx_image_name); bootvx ");

  env = getenv("vx_flash");
  if (!env)
  {
    char buf[512];
    sprintf(buf, "setenv bootargs mgi(0,0) host:VxWorks h=$(serverip) e=$(ipaddr):FFFF0000 u=anonymous pw=target; setenv bootaddr 0x700; vx_unc_boot");
    setenv ("vx_flash", buf);
  }
...
int soc_power_clock_init(void)
{
    if (mvSocPowerClockInit() != MV_OK)
    {
        mvOsPrintf("%s: mvSocPowerClockInit failed.\n", __func__);
        return 1; /* failure */
    }
...
int mv_set_power_scheme(void)
{
  ...
  if (devId == MV_6180_DEV_ID || boardId == RD_88F6281A_PCAC_ID)
  {
    /* Sata power down */
...
{
    MV_SFLASH_INFO *sFlashP = getMvFlashInfo(BOOT_FLASH_INDEX);
    MV_U32 offset = (MV_U32)env_ptr - sFlashP->baseAddr;
    mvSFlashBlockRd(sFlashP, offset, (MV_U8 *)buffer, size);
}
#endif
