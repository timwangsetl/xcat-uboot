...
//	mvIntrfaceParamPrint();
	...
	for(i = 0; i< MV_DRAM_MAX_CS; i++)
	{
		...
		if (i == 0)
		{
			if (gd->bd->bi_dram[i].size == 256<<20)
			{
				//
				// set memory at 0 to pi_value and compare to 0x8000000 (= 128MB)
				// if it's equal then we have duplication, which means that the memory 
				// is only 128 MB
				//
				*(MV_U32 *)0 = pi_value; // 
				if (*(MV_U32 *)0x8000000 == pi_value)
				{
//					printf("DRAM configured 256MB, probed 128MB\n");
					MV_REG_WRITE(SDRAM_SIZE_REG(0,0), 0x07fffff1); // set size to 128MB
					dramTotalSize = gd->bd->bi_dram[i].size = 128<<20;
				}
			}
		}
